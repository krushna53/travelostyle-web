  ...
  return (
    <section className="w-full py-16 px-4 md:px-12 bg-white font-sans text-stone-900">
      <div className="max-w-6xl mx-auto text-left md:text-center mb-16">
        <p className="font-taprom whitespace-nowrap md:whitespace-normal font-normal text-[#1A1A1A] md:text-stone-600 mb-2 leading-[40px] md:leading-normal tracking-[0.05em] md:tracking-wide responsive-text-size-[28px]">
          {capitalizeFirstLetter(subheading)}
        </p>{" "}
        <h2 className="font-nohemi max-w-[336px] md:max-w-none md:whitespace-normal sm:text-4xl  font-semibold md:font-bold leading-[40px] md:leading-tight tracking-[0.05em] md:tracking-tight text-[#1A1A1A] md:text-black responsive-text-size-[28px]">
          {mainHeading}
        </h2>
      </div>
      <div className="flex flex-wrap justify-center gap-6 max-w-7xl 2xl:max-w-[1704px] mx-auto">
        {steps?.map((step, index) => (
          <div
            key={index}
            className="p-8 rounded-[10px] border-2 border-[#1A1A1A] flex flex-col justify-between transition-shadow hover:shadow-md max-w-[336px] w-full max-md:min-h-[305px] sm:w-[calc(50%-12px)] lg:w-[calc(33.333%-16px)] min-[1920px]:w-[407px] min-h-[380px]"
            style={{ backgroundColor: step.bgColor || "#ffffff" }}
          >
            <div>
              <span className="block font-nohemi md:text-4xl font-semibold md:font-extrabold tracking-[0.05em] md:tracking-normal text-[#000000] md:text-black mb-6 responsive-text-size-[28px]">
                {step.number}
              </span>
