...
export async function getAllJourneysList() {
  ...
  // every page.
  let nextUrl = `${API_BASE_URL}/jsonapi/node/journey?filter[status][value]=1&include=field_journey_image.field_media_image,field_journey_tag,field_month,field_category,field_region,field_starts_in`;
  let allData = [];
  let allIncluded = [];
  ...
  return allData.map((item, index) => {
    ...
    const imageUrl = buildFileUrl(rawUrl) || "";
