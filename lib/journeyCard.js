...
async function getNearestDeparturesByJourney() {
  try {
    const res = await fetch(`${API_CLIENT_BASE}/jsonapi/node/book_your_journey?filter[status][value]=1`, {
      cache: "no-store",
    });
    if (!res.ok) return {};
...
export async function getJourneyCards() {
  const [res, nearestDeparturesByJourney] = await Promise.all([
    fetch(`${API_CLIENT_BASE}/jsonapi/node/journey?filter[status][value]=1&include=${INCLUDE}&sort=-changed`, {
      cache: "no-store",
    }),
